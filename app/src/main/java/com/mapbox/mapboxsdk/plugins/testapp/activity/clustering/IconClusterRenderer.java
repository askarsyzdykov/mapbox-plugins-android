 ...
 * </p>
 */
public class IconClusterRenderer<T extends IconClusterItem> implements ClusterRenderer<T> {
    ...
    public IconClusterRenderer(Context context, MapboxMap map, ClusterManagerPlugin<T> clusterManagerPlugin) {
        mMap = map;
        mAnimate = true;
    ...
    /**
     * Transforms the current view (represented by IconClusterRenderer.mClusters and IconClusterRenderer.mZoom) to a
    ...
    private class RenderTask implements Runnable {
        ...
        @SuppressLint("NewApi")
        public void run() {
            if (clusters.equals(IconClusterRenderer.this.mClusters)) {
                mCallback.run();
                return;
            }
            ...
            if (IconClusterRenderer.this.mClusters != null && SHOULD_ANIMATE) {
                existingClustersOnScreen = new ArrayList<Point>();
                for (Cluster<T> c : IconClusterRenderer.this.mClusters) {
            ...
            mMarkers = newMarkers;
            IconClusterRenderer.this.mClusters = clusters;
            mZoom = zoom;
